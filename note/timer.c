...
void timer(){
    ...
    TH1 = ((65536-5000) *  12  / 11.0592) / 256; //  (2 ** 16 - X) * 12 /11.0592
    TL1 = ((65536-5000) * 12 / 11.0592) % 256;// 11.0592 foc
    ET0 = 1; //允许中断
    TR0 = 1;// 开启定时器0
    EA = 1;//总中断
}
...
void timer0_sus() interrupt 1 using 0{
    ...
    TH1 = ((65536-5000) *  12  / 11.0592) / 256; // 11.0592 foc
    TL1 = ((65536-5000) * 12 / 11.0592) % 256;
}
